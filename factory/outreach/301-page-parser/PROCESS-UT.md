...
```bash
# Interrupt the running Python process (local execution):
kill $(pgrep -f "page-parser.py")

# Verify no slots were left in a partial write state:
wrangler d1 execute svg-d1-outreach-ops --command "SELECT COUNT(*) FROM slot_workbench WHERE person_source = 'page_parser_301' AND person_first_name IS NULL"
```
