...
| OSAM Section | doctrine/OSAM.md -- outreach execution (04.04.04) |
...
|-----------|------|-------------------|
| 11-structured-data | Cloudflare D1 | CID reads, campaign queue, MID state, event log |
| 22-webhook-gateway | CF Workers + Hono | Inbound delivery webhooks from Mailgun/HeyReach |
| 15-scheduling | CF Cron Triggers | Future: scheduled batch sends, daily cap management |
...
|------|----------------|-----------|-------------|-------------|
| Cloudflare D1 | 11-structured-data | Free | D1 binding | Campaign model evaluation, sequence building, event logging |
| Webhook receiver | 22-webhook-gateway | Free | none | Inbound delivery webhooks from Mailgun/HeyReach |
| CF Cron Triggers | 15-scheduling | Free | none | Daily batch send scheduling and cap management |
| Composio | (integration vendor) | Cheap | COMPOSIO_API_KEY (Doppler) | Routes MIDs to Mailgun or HeyReach based on channel tag |
| Mailgun | (delivery vendor) | Cheap | MAILGUN_API_KEY (Doppler) | Email delivery — uses LCS pipeline's 14-domain rotation |
| HeyReach | (delivery vendor) | Cheap | HEYREACH_API_KEY (Doppler) | LinkedIn outreach delivery |
