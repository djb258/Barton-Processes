...
| CTB Position | factory/svg-agency/200-people-worker |
| ORBT | BUILD |
| Strikes | 0 |
...
|------|------|-----------|-------------|-------------|
| D1 queries | Native | Free | Auto | Pass 1 — staging promotion + blog data consumption |
| Startpage | Search engine | Free | None (via proxy) | Pass 2 — LinkedIn profile discovery via search index |
| DataImpulse | Residential proxy | Cheap (~$1-2/month) | PROXY_GATEWAY_URL, PROXY_API_KEY (Doppler) | Pass 2 — routes Startpage through residential IPs |
| Brave Search API | Search API | Top Shelf ($3-5/1K) | BRAVE_API_KEY (Doppler) | Pass 3 — backup search source for remaining empty slots |
