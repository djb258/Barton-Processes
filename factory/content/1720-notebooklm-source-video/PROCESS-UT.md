...
| 2  | OSAM — READ / WRITE / Join Chain / Forbidden Paths / Query Routing / Process Composition      | [x]    | §5, §6                                      |
| 3  | Component Status — every dependency has a status indicator with 1-line state                  | [x]    | §3                                          |
| 4  | Owner — human who fixes this at 2 AM                                                          | [x]    | §1 — Dave Barton                            |
| 5  | Live Dashboard — URL or explicit "N/A"                                                        | [x]    | §3 — N/A (browser-automation process)       |
...
| 12 | Live Verification — every numeric count, cron, URL, command, BAR status grounded             | [ ]    | §9b — pending first successful end-to-end   |
...
|-----------------------------------------|--------------------|------------------------------------------------------------------------|--------------|
| Chrome DevTools MCP                     | browser automation | Opens, navigates, fills, and interacts with NotebookLM web UI          | GREEN — mcp__chrome-devtools tools available |
| NotebookLM Studio (notebooklm.google.com) | provider surface  | Notebook creation, source ingestion, video artifact generation          | GREEN — accessible via Chrome session |
| Dave's Google account (logged-in Chrome session) | auth       | Provides notebook access; no raw secret stored in this UT             | GREEN — session must be active before run |
| Local filesystem — Downloads folder     | storage            | Receives downloaded video artifact                                     | GREEN — standard OS path |
| PROC-1750 Video Picker                  | upstream           | Dispatches source_packet with path_id = notebooklm_source_video        | BUILD — not yet certified |
| PROC-1800 CF Stream Upload              | downstream         | Consumes artifact_download_path + CF Stream handoff metadata           | BUILD — 1 strike |
| LBB (lbb.svg-outreach.workers.dev)      | knowledge store    | Receives artifact lineage records per run                              | GREEN |
...
| 9    | artifact_id, source_packet_id | Build CF Stream handoff metadata packet; write to handoff file                      | handoff_file_path               | structured write        |
...
| Handoff JSON file             | artifact_download_path, notebook_id, artifact_id, CF Stream metadata | Step 9   |
...
| CF Stream handoff metadata  | D-1720-HSM       | JSON object     | Structured packet written to handoff_file_path for PROC-1800 to consume    | C (structure) |
...
| CF Stream handoff metadata  | §4 Output, §5 WRITE | structured pass to PROC-1800                          |
...
| Handoff contract shape             | CF Stream handoff metadata JSON (D-1720-HSM schema)       |
...
| handoff_file_path         | filesystem path | Path where CF Stream handoff metadata JSON is written |
...
6. Handoff file written to handoff_file_path; JSON is valid and contains all required fields
...
|--------------------------------|----------------|--------------------------|---------------|---------|
| Chrome session active          | boolean        | true                     | PENDING       | RED — not yet run |
| sources_loaded_count match     | integer check  | == source_packet.count   | PENDING       | RED — not yet run |
| generation_complete            | boolean        | true within wait window  | PENDING       | RED — not yet run |
| artifact_download_path exists  | filesystem     | file exists, size > 0    | PENDING       | RED — not yet run |
| handoff_file_path valid        | JSON check     | all required fields present | PENDING    | RED — not yet run |
| LBB record_id returned         | API response   | non-null UUID            | PENDING       | RED — not yet run |

**Verification note:** All gauges are RED (no successful end-to-end run completed during BUILD). First live run will set baseline values.
...
| handoff delivery rate   | PENDING | PENDING | PENDING | TIGHTENING expected | certify if 1.0 x3 |

### 10c. ORBT Gate Rules

| From              | To                | Gate                                                                    |
|-------------------|-------------------|-------------------------------------------------------------------------|
| BUILD             | OPERATE           | First 3 end-to-end runs complete; all gauges GREEN; auditor sign-off   |
| OPERATE           | REPAIR            | Any gauge RED on two consecutive runs                                   |
...
| error_code   | null                                                      |
| error_message | null                                                     |
| tools_used   | atlas/HOW_TO_BUILD_A_CHILD_UT.md, atlas/PROCESS_FILL_INSTRUCTIONS.md, atlas/constants/UNIFIED_TEMPLATE.md, docs/plans/BAR-VIDEO-LANE-UTS.plan.md, 1700-video-ctb/PROCESS-UT.md, 1800-cf-stream-upload/PROCESS-UT.md |
| duration_ms  | PENDING                                                   |
| cost_cents   | PENDING                                                   |
...
|---------------------------------------|---------------------------------------------|----------------------------------------------|
| Plan Book                             | docs/plans/BAR-VIDEO-LANE-UTS.plan.md       | Process scope, BAR-388, access matrix, six fills |
| Child UT guide                        | atlas/HOW_TO_BUILD_A_CHILD_UT.md            | Six fills and section landing zones          |
| Process Fill Instructions             | atlas/PROCESS_FILL_INSTRUCTIONS.md          | Section fill rules and acceptance criteria   |
| Parent CTB process                    | 1700-video-ctb/PROCESS-UT.md               | HEIR pattern and Geometry model              |
| Downstream process                    | 1800-cf-stream-upload/PROCESS-UT.md        | Handoff contract shape                       |
...
| Last Modified | 2026-05-04                                    |
| Version       | 0.1.0                                         |
| Template      | UNIFIED_TEMPLATE.md + PROCESS_FILL_INSTRUCTIONS.md |
| Status        | BUILD                                         |
| Authority     | Gated (CC-03)                                 |
| BAR           | BAR-388                                       |
