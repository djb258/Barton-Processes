...
outside:
  heir:
    sovereign_ref: barton-processes
    hub_id: bar-600-conform-wire
    ctb_placement: leaf
    ctb_node: barton-enterprises/svg-agency/outreach/600-bit-scoring/bars/conform-wire
    imo_topology: hub
    cc_layer: CC-01
...
inside:
  heir:
    process_id: bar.600-conform-wire
    species: Plan-Body
    version: "1.0.0"
    last_modified: "2026-05-06"
    companion_manifest: docs/plans/BAR-600-CONFORM-WIRE/PLAN-BOOK.md
  orbt:
    library_state: BUILD
---

# PLAN BOOK — BAR-600-CONFORM-WIRE

## §1 Purpose

Bring `factory/outreach/600-bit-scoring/` to Atlas/BS Law Y-junction conformance and
wire its paired Library Books (PROCESS-UT.md + workflow.yaml) into Mission Control
via the `system.processes` shelf slot.

Process: **BIT Scoring** (PROC-600)
...
mission_control_wiring:
  - artifact: factory/outreach/600-bit-scoring/PROCESS-UT.md
    disposition: WIRE
    target_slot_heir_id: imo-creator.mission-control.system.processes
    rationale: |
      UT-Body doctrine artifact for PROC-600 BIT Scoring. Surfaced via the
  ...
  - artifact: factory/outreach/600-bit-scoring/workflow.yaml
    disposition: WIRE
    target_slot_heir_id: imo-creator.mission-control.system.processes
    rationale: |
