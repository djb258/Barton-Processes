# MECHANIC-INPUT-GAPS — bp.300 Blog Worker
**BAR:** BAR-MONDAY-16-FLEET-GREEN
**Audited:** 2026-05-08
**Auditor:** Sonnet (read-only baseline)
**Process path:** `factory/outreach/300-blog-worker/`
...
| §9b ≥1 real value (no TBD/TBV-only) | G06 | PASS | 7 rows with real values present |
...
### GAP-300-01 — G03: `services` missing from outside.heir
- **Fault:** `services` field absent from the outside.heir frontmatter block in PROCESS-UT.md
- **Fix:** Add `services: [blog-worker, cloudflare-d1, lbb, mission-control]` (verify against wrangler.toml) to outside.heir
- **Scope:** PROCESS-UT.md outside.heir block

### GAP-300-02 — G08: §14 SESSION LOG wrong columns
