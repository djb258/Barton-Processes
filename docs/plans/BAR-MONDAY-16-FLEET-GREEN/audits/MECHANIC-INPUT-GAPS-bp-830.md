# MECHANIC-INPUT-GAPS — bp.830 Client Portal
**BAR:** BAR-MONDAY-16-FLEET-GREEN
**Audited:** 2026-05-08
**Auditor:** Sonnet (read-only baseline)
**Process path:** `factory/client/830-client-portal/`
...
| 8 HEIR fields in outside.heir | G03 | FAIL | `services` and `subject_id` fields absent from PROCESS-UT.md outside.heir frontmatter |
| Valid 4-state ORBT | G04 | PASS | |
| ctb_node anchored on barton-enterprises/... | G05 | PASS | |
| §9b ≥1 real value (no TBD/TBV-only) | G06 | FAIL | All 8 §9b rows are TBV — no live verification performed |
...
### GAP-830-01 — G03: `services` and `subject_id` missing from outside.heir
- **Fault (a):** `services` field absent from the outside.heir frontmatter block in PROCESS-UT.md
- **Fault (b):** `subject_id` field absent from the outside.heir frontmatter block in PROCESS-UT.md
- **Fix (a):** Add `services: [client-portal, cloudflare-d1, lbb, mission-control]` (verify against wrangler.toml) to outside.heir
- **Fix (b):** Add `subject_id: svg-client-proc` to outside.heir
- **Scope:** PROCESS-UT.md outside.heir block

### GAP-830-02 — G06: §9b all-TBV
- **Fault:** All 8 §9b live verification rows have TBV in the Value column — no live verification performed
...
- **Scope:** PROCESS-UT.md §9b — all 8 TBV rows

### GAP-830-03 — G08: §14 SESSION LOG wrong columns
